...
var Bravia = function(ip, callback) {
  ...
  this.cookieJar = request.jar(new FileCookieStore(this.cookiePath));
  this.commands = {};
...
Bravia.prototype.exec = function(command, callback) {
  ...
  this.auth(function() {
    that.getCommandCode(command, function(code) {
      that.makeCommandRequest(code, callback);
    });
  });
...
Bravia.prototype.request_with_error = function(options, callback) {
...
Bravia.prototype.updateCookieJar = function(callback) {
  var data = fs.readFileSync(this.cookiePath, 'utf8');
  data = data.replace('\/sony\/', '\/sony');
  fs.writeFileSync(this.cookiePath, data);
  this.cookieJar = request.jar(new FileCookieStore(this.cookiePath));
  callback();
}
